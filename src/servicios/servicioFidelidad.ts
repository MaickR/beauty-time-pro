...
export async function obtenerConfigFidelidad(estudioId: string): Promise<ConfiguracionFidelidad> {
  const respuesta = await peticion<{ datos: ConfiguracionFidelidad }>(
    `/estudio/${estudioId}/fidelidad/config`,
  );
...
export async function obtenerRankingFidelidad(
  estudioId: string,
  limite = 10,
): Promise<ClienteRankingFidelidad[]> {
  const respuesta = await peticion<{ datos: ClienteRankingFidelidad[] }>(
    `/estudio/${estudioId}/fidelidad/ranking?limite=${limite}`,
  );
  return respuesta.datos;
}

export async function obtenerFidelidadCliente(
  clienteId: string,
  estudioId: string,
): Promise<EstadoFidelidadCliente> {
  const respuesta = await peticion<{ datos: EstadoFidelidadCliente }>(
    `/clientes/${clienteId}/fidelidad/${estudioId}`,
  );
  return respuesta.datos;
}
...
export async function buscarClienteFidelidadPorTelefono(
  ...
  const coincidencia = await peticion<{ datos: EstadoFidelidadCliente | null }>(
    `/estudio/${estudioId}/fidelidad/cliente?telefono=${encodeURIComponent(telefono)}`,
  );
  return coincidencia.datos;
}
