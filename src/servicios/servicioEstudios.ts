 ...
 */
import type { Estudio, Servicio, Personal } from '../tipos/index';
import { peticion } from '../lib/clienteHTTP';

type RespuestaEstudio = { datos: Estudio };

/** Crea un estudio nuevo. */
export async function guardarEstudio(_id: string, datos: Omit<Estudio, 'id'>): Promise<void> {
  await peticion('/estudios', {
    method: 'POST',
    body: JSON.stringify({
      nombre: datos.name,
...
export async function actualizarEstudio(id: string, campos: Partial<Estudio>): Promise<void> {
  ...
  if (campos.assignedKey !== undefined) cuerpo['claveDueno'] = campos.assignedKey;
  if (campos.clientKey !== undefined) cuerpo['claveCliente'] = campos.clientKey;
  if (campos.subscriptionStart !== undefined) cuerpo['inicioSuscripcion'] = campos.subscriptionStart;
  if (campos.paidUntil !== undefined) cuerpo['fechaVencimiento'] = campos.paidUntil;
...
/** Actualiza la lista de personal de un estudio. */
export async function actualizarStaff(id: string, staff: Personal[]): Promise<void> {
  await peticion<RespuestaEstudio>(`/estudios/${id}`, {
    method: 'PUT',
