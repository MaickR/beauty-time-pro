...
export function ConfirmacionReserva({ nombreCliente, descripcionRecompensa, onCerrar }: PropsConfirmacionReserva) {
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="bg-white rounded-[3rem] p-10 max-w-lg w-full text-center shadow-2xl">
        ...
        <h2 id="tituloConfirmacion" className="text-3xl font-black italic uppercase tracking-tighter mb-4 text-slate-800">¡Cita Confirmada!</h2>
        <p className="text-lg font-medium text-slate-600 mb-8">
          Muchas gracias por tu preferencia, <strong className="text-pink-600 uppercase">{nombreCliente}</strong>. Tu especialista te estará esperando.
        </p>
        ...
        {descripcionRecompensa && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-3xl p-6 mb-8 text-left shadow-sm">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h4 className="font-black text-emerald-800 uppercase text-xs tracking-widest mb-1">Recompensa Ganada</h4>
                <p className="text-emerald-900 text-sm font-medium leading-relaxed">
                  🎉 ¡Felicidades {nombreCliente}! Has ganado una recompensa: <strong>{descripcionRecompensa}</strong>.
                </p>
              </div>
            </div>
          </div>
