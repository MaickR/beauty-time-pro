...
export function SelectorServicio({ estudio, personalSeleccionado, serviciosSeleccionados, moneda, onAlternar }: PropsSelectorServicio) {
  const miembro = estudio.staff?.find((s) => s.id === personalSeleccionado);
  const serviciosDisponibles = estudio.selectedServices.filter((s) => miembro?.specialties.includes(s.name));
  ...
  return (
    <section className="bg-slate-50 rounded-[3rem] p-8 md:p-10 border border-slate-200 shadow-sm">
      ...
      <p className="text-xs font-bold text-slate-500 uppercase mb-4">
        Servicios que realiza <strong>{miembro?.name ?? 'este especialista'}</strong>:
      </p>

      {serviciosDisponibles.length === 0 ? (
        <p className="text-slate-400 italic font-bold">El especialista no tiene tratamientos habilitados en este momento.</p>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex flex-col">
                  <span className={`font-black text-sm uppercase ${seleccionado ? 'text-white' : 'text-slate-800'}`}>
                    {seleccionado && '✓ '}{servicio.name}
                  </span>
                  <span className={`text-[10px] font-bold mt-1 flex items-center gap-1 ${seleccionado ? 'text-pink-200' : 'text-slate-400'}`}>
                    <Clock className="w-3 h-3" /> {servicio.duration} MINUTOS
                  </span>
                </div>
                {servicio.price > 0 && (
                  <span className={`font-black text-lg shrink-0 ${seleccionado ? 'text-white' : 'text-green-600'}`}>
      ...
      {serviciosSeleccionados.length > 0 && (
        <div className="mt-8 bg-slate-900 text-white p-6 rounded-3xl flex justify-between items-center shadow-xl">
          <div>
            ...
            <p className="text-3xl font-black text-green-400">{formatearDinero(totalPrecio, moneda)}</p>
          </div>
