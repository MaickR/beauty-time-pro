...
import { Store, LogOut, Calendar, Wallet, DollarSign, PieChart, TrendingUp, Users, Palette, Gift } from 'lucide-react';
...
export function PaginaAdminEstudio() {
  ...
  const [seccion, setSeccion] = useState<'ingresos' | 'clientes' | 'fidelidad' | 'salon'>('ingresos');
  ...
  const totalHoy = completadas.filter((b) => b.date === hoySrt).reduce((acc, b) => acc + (b.totalPrice ?? 0), 0);
  const totalMes = completadas.filter((b) => b.date.startsWith(mesPrefijo)).reduce((acc, b) => acc + (b.totalPrice ?? 0), 0);
  const totalAno = completadas.filter((b) => b.date.startsWith(anoPrefijo)).reduce((acc, b) => acc + (b.totalPrice ?? 0), 0);

  const tarjetas = [
    { titulo: 'Ingresos Hoy', monto: totalHoy, icono: DollarSign, color: 'bg-green-100 text-green-700' },
    { titulo: 'Ingresos Mes', monto: totalMes, icono: PieChart, color: 'bg-blue-100 text-blue-700' },
    { titulo: 'Ingresos Año', monto: totalAno, icono: TrendingUp, color: 'bg-pink-100 text-pink-700' },
  ];
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
        <div className="flex bg-slate-200/50 p-1 rounded-2xl w-fit border border-slate-200 mx-auto md:mx-0">
          <button onClick={() => navegar(`/estudio/${estudio.id}/agenda`)} className="px-4 md:px-6 py-3 rounded-xl text-[10px] md:text-xs font-black transition-all flex items-center gap-2 text-slate-500 hover:text-slate-800">
            <Calendar className="w-4 h-4" /> Agenda & Personal
          </button>
          <button onClick={() => navegar(`/estudio/${estudio.id}/admin`)} className="px-4 md:px-6 py-3 rounded-xl text-[10px] md:text-xs font-black transition-all flex items-center gap-2 bg-white shadow-sm text-slate-900">
        ...
        {seccion === 'ingresos' && (
          <>
            <h2 className="text-3xl font-black italic uppercase tracking-tighter">Estadísticas de Ingresos</h2>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {tarjetas.map(({ titulo, monto, icono: Icono, color }) => (
                <div key={titulo} className="bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-sm">
                  <div className={`${color} w-12 h-12 rounded-2xl flex items-center justify-center mb-4`}><Icono className="w-6 h-6" /></div>
                  <p className="text-xs font-black text-slate-400 uppercase tracking-widest">{titulo}</p>
                  <p className="text-3xl font-black mt-1 tracking-tighter">{formatearDinero(monto, moneda)}</p>
                </div>
              ))}
            </div>
        ...
        {seccion === 'clientes' && (
          <>
            <h2 className="text-3xl font-black italic uppercase tracking-tighter">Directorio de Clientes</h2>
