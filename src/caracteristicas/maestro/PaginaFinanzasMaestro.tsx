...
export function PaginaFinanzasMaestro() {
  ...
  const { estudios, pagos } = usarContextoApp();
    ...
    <div className="min-h-screen bg-slate-50 pb-20 font-sans">
      ...
      <main className="max-w-7xl mx-auto p-8">
        <div className="flex bg-slate-200/50 p-1 rounded-2xl w-fit mb-8 border border-slate-200">
          <button onClick={() => navegar('/maestro')} className="px-6 py-3 rounded-xl text-xs font-black transition-all flex items-center gap-2 text-slate-500 hover:text-slate-800">
            <Store className="w-4 h-4" /> Directorio de Red
          </button>
          <button onClick={() => navegar('/maestro/finanzas')} className="px-6 py-3 rounded-xl text-xs font-black transition-all flex items-center gap-2 bg-white shadow-sm text-slate-900">
            <PieChart className="w-4 h-4" /> Estado de Cuenta
          </button>
        </div>

        <h1 className="text-4xl font-black italic uppercase tracking-tighter mb-8">Estado de Cuenta</h1>
