...
import { X, Download, AlertTriangle, Ban, Pencil, Eye, EyeOff, RefreshCw } from 'lucide-react';
import {
...
export function ModalControlSalones({ onCerrar }: PropsModalControlSalones) {
  ...
  const guardarEditar = () => {
    ...
    if (formEditar.contrasena && !esContrasenaFormatoSalonValida(formEditar.contrasena)) {
      mostrarToast({
        mensaje: `La nueva contraseña no cumple el patrón. ${DESCRIPCION_FORMATO_CONTRASENA_SALON}.`,
        variante: 'error',
      });
      return;
    }
                      ...
                      {activos.data?.datos.map((s: SalonActivo) => (
                        <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50">
                          ...
                          <td className="py-3 px-2">
                            <span
                              className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase ${s.plan === 'PRO' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-700'}`}
                            >
                              {s.plan}
                            </span>
                          </td>
                    ...
                    <tbody>
                      {suspendidos.data?.datos.map((s: SalonSuspendido) => (
                        <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50">
                          ...
                          <td className="py-3 px-2">
                            <Tooltip texto="Reactivar">
                              <button
                                onClick={() => mutActivar.mutate(s.id)}
                                aria-label={`Reactivar ${s.nombre}`}
                                className="p-2 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 transition-colors hover:bg-emerald-100"
                              >
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setFormEditar((f) => ({
                        ...f,
                        contrasena: generarContrasenaSalon(modalEditar.nombre, modalEditar.dueno),
                      }))
                    }
