...
import { FlaskConical, RefreshCcw, Calendar, Settings2, Users, ShoppingBag } from 'lucide-react';
import {
      ...
      <section className="grid gap-4 lg:grid-cols-[1.25fr_0.95fr]">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-600">
                Usa este salón aislado para presentar la agenda real y el flujo administrativo sin
                tocar información productiva.
              </p>
            </div>
        ...
        <article className="rounded-4xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <ul className="mt-4 space-y-3 text-sm text-slate-600">
            <li>
              El reinicio elimina reservas, clientes, productos, personal y pagos creados dentro del
              salón demo.
            </li>
            <li>Tus ventas asignadas y salones productivos no se modifican.</li>
            <li>Usa el reinicio antes de cada sesión comercial para mantener limpio el relato.</li>
          </ul>
          <div className="mt-6 rounded-3xl bg-slate-100 p-4">
            <p className="text-xs font-bold uppercase text-slate-500">Última sincronización</p>
            <p className="mt-1 text-sm font-semibold text-slate-900">
              {new Date(salonDemo.actualizadoEn).toLocaleString()}
            </p>
            <p className="mt-3 text-xs font-bold uppercase text-slate-500">Renovación</p>
            <p className="mt-1 text-sm font-semibold text-slate-900">
              {salonDemo.fechaVencimiento}
            </p>
          </div>
        </article>
      </section>
