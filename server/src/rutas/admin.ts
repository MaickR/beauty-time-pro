...
export async function rutasAdmin(servidor: FastifyInstance): Promise<void> {
  ...
  /**
   * POST /admin/produccion/reiniciar-datos — reinicia datos operativos
   * Requiere confirmación explícita para evitar ejecuciones accidentales.
   */
  servidor.post(
    '/admin/produccion/reiniciar-datos',
    { preHandler: [verificarJWT, requierePermiso('crearAdmins')] },
    async (solicitud, respuesta) => {
      const confirmacion = z
        .object({ confirmacion: z.literal('REINICIAR_PRODUCCION') })
        .safeParse(solicitud.body);

      if (!confirmacion.success) {
        return respuesta.code(400).send({ error: 'Confirmación inválida' });
      }
      ...
      try {
        ...
        const sesionesAutenticacion = await prisma.sesionAutenticacion.deleteMany({});
        const correosPendientes = await prisma.correoPendiente.deleteMany({});
        const suscripcionesPush = await prisma.suscripcionPush.deleteMany({});
        const tokensReset = await prisma.tokenReset.deleteMany({});
        ...
        resumen = {
          ...
          sesionesAutenticacion: sesionesAutenticacion.count,
          correosPendientes: correosPendientes.count,
          suscripcionesPush: suscripcionesPush.count,
          tokensReset: tokensReset.count,
      ...
      await registrarAuditoria({
        usuarioId: payload.sub,
        accion: 'reiniciar_datos_produccion',
        entidadTipo: 'sistema',
      ...
      return respuesta.send({
        datos: {
          mensaje: 'Datos operativos reiniciados correctamente',
          resumen,
        },
      });
    },
